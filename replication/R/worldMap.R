...
#     rect(xleft=-171.5, xright=-130, ytop=-31, ybottom=-59)
  ...
  if (nval > 9) {
    ...
    if (is.null(miny)) miny <- min(na.omit(world@data[, x]))
    colorpal <- brewer.pal(9, 'Reds')
    colorpal <- colorRampPalette(colorpal)
    
    breaks <- c(miny, round(maxy*1/4), round(maxy/2), round(maxy*3/4), maxy)
    ...
    colorlegend(posy=c(0.1, 0.45), posx=c(0.12, 0.14),
                col=colorpal(100), zlim=c(miny, maxy), zval=breaks,
